...
class Player1(pygame.sprite.Sprite):
    ...
    def update(self):
        ...
        if key_pressed[pygame.K_LEFT] and self.x > 0:
            self.aclx = -acl
        if key_pressed[pygame.K_RIGHT] and self.x < 500:
            self.aclx = acl
        if key_pressed[pygame.K_UP] and self.y > 0:
            self.acly = -acl
        if key_pressed[pygame.K_DOWN] and self.y < 500:
            self.acly = acl
...
class Player2(pygame.sprite.Sprite):
    ...
    def update(self):
        ...
        if self.aclx ** 2 + self.acly ** 2 > acl ** 2:  # 單位向量
            self.aclx *= abs(acl / (math.sqrt(self.aclx ** 2 + self.acly ** 2)))
            self.acly *= abs(acl / (math.sqrt(self.aclx ** 2 + self.acly ** 2)))
...
while running:
    ...
    message = font.render('blue_loc: {0},{1} | red_loc: {2},{3}'.format(int(x1), int(y1), int(x2), int(y2)), True, color.cyan)
